...
export default function App() {
  ...
  const [mediaFilter, setMediaFilter] = useState('all') // 'all' | 'post' | 'video'
  ...
  const filtered = useMemo(() => {
    let arr = activeCategory === 'all' ? posts : posts.filter(p => p.category === activeCategory)
    if (mediaFilter === 'video') arr = arr.filter(p => p.mediaType === 'youtube')
    else if (mediaFilter === 'post') arr = arr.filter(p => p.mediaType !== 'youtube')
    const pinned = arr.filter(p => p.pinned)
    const unpinned = arr.filter(p => !p.pinned).sort((a, b) => b.createdAt - a.createdAt)
    return [...pinned, ...unpinned]
  }, [posts, activeCategory, mediaFilter])
        ...
        <div className="main">
          <div className="top-bar">
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <VisitorCounter />
              <LanguagePicker />
              <button className="btn btn-ghost" onClick={() => setTweaksOpen(o => !o)}>{t('tweaks')}</button>
            </div>
          </div>
          ...
          <div className="feed-filter-bar">
            {[
              ...
              { id: 'post', label: t('postFilter') },
              { id: 'video', label: t('videoFilter') },
            ].map(f => (
              <button
                key={f.id}
                className={`feed-filter-btn${mediaFilter === f.id ? ' active' : ''}`}
          ...
          <div className={`feed ${layoutClass}${switching ? ' switching' : ''}`} ref={feedRef}>
            {filtered.length === 0 && (
              <div className="empty-state">
                <div className="empty-state-title">{t('emptyFeed')}</div>
                <div className="empty-state-sub">{t('emptyFeedSub')}</div>
              </div>
            )}
            {filtered.map((p, i) => (
              <PostCard
